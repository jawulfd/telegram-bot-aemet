...
def get_weather(token, city, days):
    now = datetime.datetime.now() + datetime.timedelta(days=int(days))

    return_text = ''

    url = "https://opendata.aemet.es/opendata/api/prediccion/especifica/municipio/diaria/" + \
          ConfMgt.get_city_id(city.upper())
    ...
    return_text += f'######## Fecha: {now.strftime("%Y-%m-%d")} ########\n\n'
    ...
    temperature_text = '\nLa temperatura máxima será de ' + str(maximum_temperature) + ' ºC y la mínima de ' \
                       + str(minimum_temperature) + ' ºC'
